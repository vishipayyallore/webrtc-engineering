...
3. Python tools: `uv sync` at repo root; `uv run …` per `pyproject.toml` / CI.
...
10. Run the **ci-checks** skill (Python tools + markdownlint + optional Lychee).
...
- `.archive/` is preserved legacy content; do not flag it as stale active content unless Swamy asks to migrate it.
